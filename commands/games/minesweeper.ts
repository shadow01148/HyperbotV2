...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (isGameActive(userId)) {
            await interaction.reply({ content: "You already have an active game!", flags: MessageFlags.Ephemeral });
            return;
        }
        ...
        collector.on("collect", async (button: ButtonInteraction) => {
            try {
                ...
                if (board[row][col] === "💥") {
                    ...
                    await button.message.edit({ components: renderBoard(board, revealed, size) });
                }
            } catch (error) {
