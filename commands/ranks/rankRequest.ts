...
async function createTicket(interaction: ChatInputCommandInteraction) {
  try {
    ...
    const existingTicket = Object.values(
      ticketData2 as Record<string, Ticket>,
    ).find((ticket) => ticket.authorId === interaction.user.id);
    ...
    await interaction.reply({
      content: `Ticket created! Ticket number: ${ticketNumber}`,
      flags: MessageFlags.Ephemeral,
    });
  } catch (error) {
      ...
      subcommand
        ...
        .addStringOption((option) =>
          option
            .setName("rank")
            .setDescription(
              "The rank to be requested. Separate with commas if you want multiple ranks.",
            )
            .setRequired(true),
        )
    ...
    if (interaction.options.getSubcommand() === "request") {
      ...
      try {
        ...
        if (!ticketData2[ticketNumber]) {
          await interaction.reply({
            content: `Ticket number ${ticketNumber} not found.`,
            flags: MessageFlags.Ephemeral,
          });
          return;
        }
        ...
        await fs.writeFile(
          ticketsDataPath,
          JSON.stringify(ticketData2, null, 2),
        );
        await interaction.reply({
          content: `Ticket number ${ticketNumber} closed.`,
          flags: MessageFlags.Ephemeral,
        });
      } catch (error) {
